...
class Setting extends React.PureComponent {
      ...
      if (menuObj.children.length !== 0) {
        vdom.push(
          <li key={menuObj.id} style={{listStyleType: 'none'}}>
              <h1 id={'li'+menuObj.id} className='menu' style={{ fontSize: '14px', cursor: 'pointer', color: '#333'}}>
        ...
        if (menuObj.is_index === 'true') {
          vdom.push(
            <li key={menuObj.id} style={{listStyleType: 'none'}}>
                <h1 id={'li'+menuObj.id} className='menu' style={{ fontSize: '12px', cursor: 'pointer', color: '#666'}}>
                    <span onClick={this.dealMenu.bind(this, menuObj)}>{menuObj.name}</span>
  ...
  operateMenu(operate) {
    ...
    let portal_id =  this.props.form_data.portal[0];
    $.ajax({
      type: "POST",
      ...
      success: function(data){
        ...
        if (data) {
          location.href = '/superset/portal/' + portal_id + '/edit';
        } else {
          alert(data)
        }
      }
  ...
  upload(){
    ...
    let array = $('#file')[0].files[0].name.split('.');
    ...
    $.ajax({
      ...
      success: function(data){
        if(data === 'true') {
          $('#img').attr('src', '/static/logo/logo_' + portal_id + '_' + timeStamp + '.png');
        } else {
          alert('upload failed')
        }
      }
  ...
  render() {
    ...
    let nodes = this.getLeftMenu(0, this.props.form_data.menus);
    console.log(nodes)
        ...
        <div className="container-fluid" height={this.state.contentHeight}>
          <div style={{width: '100%', height: '85%', overflow: 'hidden'}}>
            ...
            <div style={{width: '40%',float: 'left', marginTop: '50px', marginLeft: '100px'}}>
              ...
              <div className="col-lg-12" style={{height: '50px'}}>
                <button className="btn btn-primary" onClick={this.operateMenu.bind(this, 'add')} style={{marginLeft: '20px', height: '40px'}}>添加</button>
                <button className="btn btn-success" onClick={this.operateMenu.bind(this, 'modify')} style={{marginLeft: '20px', height: '40px'}}>修改</button>
                <button className="btn btn-danger" onClick={this.operateMenu.bind(this, 'delete')} style={{marginLeft: '20px', height: '40px'}}>删除</button>
                <a className="btn btn-primary" target="_blank" href={'/superset/portal/' + this.props.form_data.portal[0] + '/show'} style={{marginLeft: '20px', height: '40px'}}>门户预览</a>
              </div>
              <div className="col-lg-12" style={{marginTop: '30px'}}>
                ...
                <div className="col-lg-1">
                  ...
                  <img id='img' style={{width: '240px', height: '120px', marginTop: '20px'}} 
                      src={'/static/logo/logo_' + this.props.form_data.portal[0] + '_' + this.props.form_data.portal[3] + '.png'} />
                </div>
              </div>
            </div>
          </div>
